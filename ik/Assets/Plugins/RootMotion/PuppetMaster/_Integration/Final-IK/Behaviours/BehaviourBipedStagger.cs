...
namespace RootMotion.Dynamics {
    ...
    public partial class BehaviourBipedStagger : BehaviourBase {
        ...
        [LargeHeader("Regaining Balance")]

        [Tooltip("If enabled, puppet will regain balance instead of falling over when all conditions below are met.")]
        /// <summary>
        /// If enabled, puppet will regain balance instead of falling over when all conditions below are met.
        /// </summary>
        public bool canRegainBalance;

        [Tooltip("Maximum center of Mass angle allowing regaining balance.")]
        /// <summary>
        /// Maximum center of Mass angle allowing regaining balance.
        /// </summary>
        public float regainBalanceAngle = 30f;

        [Tooltip("Minimum time since behaviour activation before regaining balance can be initiated.")]
        /// <summary>
        /// Minimum time since behaviour activation before regaining balance can be initiated.
        /// </summary>
        public float regainBalanceMinTime = 1f;

        [Tooltip("Minimum time since last footstep before regaining balance can be initiated.")]
        /// <summary>
        /// Minimum time since last footstep before regaining balance can be initiated.
        /// </summary>
        public float regainBalanceMinTimeSinceLastStep = 0.2f;

        [Tooltip("The speed of blending in pin weights.")]
        /// <summary>
        /// The speed of blending in pin weights.
        /// </summary>
        public float regainBalanceSpeed = 1f;
        ...
        [Tooltip("Called when the puppet is re-balanced without falling over.")]
        /// <summary>
        /// Called when the puppet has lost balance and stopped staggering.
        /// </summary>
        ...
        private float timer;
        private float regainBalanceWeight;
        private bool isRegainingBalance;
        ...
        private void Solve() {
            ...
            if (CanRebalance())
            {
                StartCoroutine(RegainBalance());
            }
        }
        
        // Can we initiate re-gaining balance?
        private bool CanRebalance()
        {
            if (!canRegainBalance) return false; // Not allowed to
            if (!puppetMaster.isAlive) return false;
            if (isRegainingBalance) return false; // Already re-gaining balance
            if (balanceLost) return false; // Already fell over
            if (angle > regainBalanceAngle) return false; // Too high COM angle, not standing straight
            if (timer < regainBalanceMinTime) return false; // Not enough time has passed since behaviour activation
            if (Time.time < lastStepTime + regainBalanceMinTimeSinceLastStep) return false; // Not enough time has passed since last footstep
            
            return true; // All good
        }

        // Re-gaining balance procedure
        private IEnumerator RegainBalance()
        {
            isRegainingBalance = true;
            regainBalanceWeight = 0f;

            while (regainBalanceWeight < 1f)
            {
                regainBalanceWeight += Time.deltaTime * regainBalanceSpeed;

                // Move the root along with the pelvis rigidbody
                Vector3 p = puppetMaster.muscles[0].rigidbody.position;
                p.y = puppetMaster.targetRoot.position.y;
                puppetMaster.targetRoot.position = Vector3.Lerp(puppetMaster.targetRoot.position, p, Time.deltaTime * 3f);
                GroundTarget(groundLayers);

                float d = 20f * Time.deltaTime * regainBalanceWeight;

                foreach (Muscle m in puppetMaster.muscles)
                {
                    // Drag
                    m.rigidbody.AddForce(-m.rigidbody.velocity * d, ForceMode.VelocityChange);

                    m.state.pinWeightMlp += Time.deltaTime * regainBalanceSpeed;
                }

                yield return null;
            }

            isRegainingBalance = false;
		...
		private int GetNextStepLegIndex(float deltaTime) {
			...
			if (angleVel < minAngle * minAngleMlpByProneDot.Evaluate(proneDot) || angle > maxAngle || timer >= maxDuration || forceFinish) {
				...
				if (!isFinished && angle > maxAngle || timer >= maxDuration || forceFinish) {
					...
					if (!balanceLost && finishTimer > 0.1f && !isRegainingBalance) {
						balanceLost = true;

                        float mW = puppetMaster.isAlive ? unbalancedMuscleWeightMlp : puppetMaster.stateSettings.deadMuscleWeight;
						foreach (Muscle m in puppetMaster.muscles) {
		...
		private void SolveArms(float deltaTime) {
			float mW = puppetMaster.isAlive? muscleWeight.GetValue(angle): puppetMaster.stateSettings.deadMuscleWeight;
			...
			float w = windmillWeight.GetValue(angle) * (1f - regainBalanceWeight);
		...
		protected override void OnActivate() {
			...
			foreach (Muscle m in puppetMaster.muscles) {
                ...
                m.state.muscleWeightMlp = puppetMaster.isAlive ? 1f : puppetMaster.stateSettings.deadMuscleWeight;
				...
				if (m.props.group == Muscle.Group.Foot) {
					...
					m.state.muscleWeightMlp = puppetMaster.isAlive? spineMuscleWeightMlp: puppetMaster.stateSettings.deadMuscleWeight;
				}
			}

...
public class bl_UCrosshair : MonoBehaviour
{
    ...
    public void FollowMouseControll(Vector2 pos)
    {
        if (RootContent == null)
            return;
        if (GetCrosshair.isStatic )
    ...
    public void SetColor(bool detact)
    {
        if (detact)
        {
            GetCrosshair.SetColor(OnTargetFocusColor, ColorTransitionDuration);
    ...
    public void SetActive(bool hide)
    {
        if (RootContent)
        {
            GetCrosshair.gameObject.SetActive(hide);
        }
    }

...
namespace RLD
{
    ...
    public class ObjectExtrudeGizmo : GizmoBehaviour
    {
        ...
        public void SetExtrudeTargets(IEnumerable<GameObject> extrudeTargets)
        {
            ...
            _targetParents.RemoveAll(item => { var objectType = item.GetGameObjectType(); return objectType != GameObjectType.Mesh && objectType != GameObjectType.Terrain && objectType != GameObjectType.Sprite; });
                    ...
                    foreach(var targetParent in _targetParents)
                    {
                        ...
                        if (!Hotkeys.EnableOverlapTest.IsActive())
                        {
                            ...
                            if (clonedHierarchy != null)
                            {
                                ...
                                clonedHierarchy.InheritHierarchyLayers(targetParent);
                            }
                        ...
                        else
                        {
                            ...
                            if (!RTScene.Get.OverlapBox(targetOBB, _sceneOverlapFilter, _objectBuffer)) continue;

...
namespace Lean.Common
{
    ...
    public class LeanChase : MonoBehaviour
    {
        ...
        public Vector3 DestinationOffset { set { destinationOffset = value; } get { return destinationOffset; } }
        ...
        public float Linear { set { linear = value; } get { return linear; } }
        ...
        public bool SetPositionOnStart { set { setPositionOnStart = value; } get { return setPositionOnStart; } }
        ...
        public virtual void SetPosition(Touch.TouchData date)
        {
            SetPositionImimme(date);

            var rotFactor = CwHelper.DampenFactor(rotDamping, Time.deltaTime);
            transform.rotation = Quaternion.Slerp(transform.rotation, Quaternion.FromToRotation(transform.up, date.worldNormal) * transform.rotation, rotFactor);
        }

        public virtual void SetPositionImimme(Touch.TouchData date)
        {
            var factor = CwHelper.DampenFactor(damping, Time.deltaTime);

            transform.position = Vector3.Lerp(transform.position, date.worldTo, factor);
            transform.position = Vector3.MoveTowards(transform.position, date.worldTo, linear * Time.deltaTime);
        }

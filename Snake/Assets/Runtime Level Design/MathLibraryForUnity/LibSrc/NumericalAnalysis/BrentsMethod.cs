...
namespace Dest
{
	namespace Math
	{
		...
		public static partial class RootFinder
		{
			...
			public static bool BrentsMethod(Func<float, float> function,
				float x0, float x1, out BrentsRoot root,
				int maxIterations = 128, 
				float negativeTolerance = Mathfex.NegativeZeroTolerance, float positiveTolerance = Mathfex.ZeroTolerance,
				float stepTolerance = Mathfex.ZeroTolerance, float segmentTolerance = Mathfex.ZeroTolerance)
			{
				root.Iterations = 0;
				...
				{
					float fDiff01 = f0 - f1, fDiff02 = f0 - f2, fDiff12 = f1 - f2;
					float invFDiff01 = 1f / fDiff01;
					float s;
					if (fDiff02 != 0f && fDiff12 != 0f)
					{
						// Use inverse quadratic interpolation.
						float infFDiff02 = 1f / fDiff02;
						float invFDiff12 = 1f / fDiff12;
						s =
					...
					float xDiffSAvr = s - 0.75f * x0 - 0.25f * x1;
					float xDiffS1 = s - x1;
					float absXDiffS1 = Mathf.Abs(xDiffS1);
					float absXDiff12 = Mathf.Abs(x1 - x2);
					float absXDiff23 = Mathf.Abs(x2 - x3);

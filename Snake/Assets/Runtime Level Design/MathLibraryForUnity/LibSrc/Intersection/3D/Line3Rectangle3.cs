...
namespace Dest
{
	namespace Math
	{
		...
		public static partial class Intersection
		{
			...
			{
				Vector3 diff = point - rectangle.Center;
				float s0 = diff.Dot(rectangle.Axis0);
				float s1 = diff.Dot(rectangle.Axis1);
				float extent;
			...
			{
				float DdN = line.Direction.Dot(rectangle.Normal);
				if (Mathf.Abs(DdN) > _dotThreshold)
				{
					// The line is not parallel to the plane, so they must intersect.
					float signedDistance = rectangle.Normal.Dot(line.Center - rectangle.Center);

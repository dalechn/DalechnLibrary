	...
	namespace Math
	{
		...
		public static partial class Intersection
		{
			...
			private static void TrianglePlaneRelations(ref Triangle3 triangle, ref Plane3 plane,
				out float dist0   , out float dist1   , out float dist2,
				...
				out int   positive, out int   negative, out int   zero)
			{
				...
				if (dist0 > Mathfex.ZeroTolerance)
				{
					sign0 = 1;
					++positive;
				}
				else if (dist0 < -Mathfex.ZeroTolerance)
				{
					sign0 = -1;
					++negative;
				}
				...
				if (dist1 > Mathfex.ZeroTolerance)
				{
					sign1 = 1;
					++positive;
				}
				else if (dist1 < -Mathfex.ZeroTolerance)
				{
					sign1 = -1;
					++negative;
				}
				...
				if (dist2 > Mathfex.ZeroTolerance)
				{
					sign2 = 1;
					++positive;
				}
				else if (dist2 < -Mathfex.ZeroTolerance)
				{
					sign2 = -1;
					++negative;
				}
			...
			private static bool TrianglePlaneRelationsQuick(ref Triangle3 triangle, ref Plane3 plane)
			{
				...
				if (dist > Mathfex.ZeroTolerance)
				{
					sign = 1;
				}
				else if (dist < -Mathfex.ZeroTolerance)
				{
					sign = -1;
				}
				else
			...
			private static bool IntersectsSegment(ref Plane3 plane, ref Triangle3 triangle, ref Vector3 end0, ref Vector3 end1, bool grazing,
				out Triangle3Triangle3Intr info)
			{
				...
				if (maxNormal == 0)
				{
					...
					info.Point0 = new Vector3(invNX * (plane.Constant - plane.Normal.y * intr0.x - plane.Normal.z * intr0.y), intr0.x, intr0.y);
					info.Point1 = info.Quantity == 2 ? new Vector3(invNX * (plane.Constant - plane.Normal.y * intr1.x - plane.Normal.z * intr1.y), intr1.x, intr1.y) : Vector3.zero;
				}
				else if (maxNormal == 1)
				{
					float invNY = 1f / plane.Normal.y;
					info.Point0 = new Vector3(intr0.x, invNY * (plane.Constant - plane.Normal.x * intr0.x - plane.Normal.z * intr0.y), intr0.y);
					info.Point1 = info.Quantity == 2 ? new Vector3(intr1.x, invNY * (plane.Constant - plane.Normal.x * intr1.x - plane.Normal.z * intr1.y), intr1.y) : Vector3.zero;
				}
				else
				{
					float invNZ = 1f / plane.Normal.z;
					info.Point0 = new Vector3(intr0.x, intr0.y, invNZ * (plane.Constant - plane.Normal.x * intr0.x - plane.Normal.y * intr0.y));
					info.Point1 = info.Quantity == 2 ? new Vector3(intr1.x, intr1.y, invNZ * (plane.Constant - plane.Normal.x * intr1.x - plane.Normal.y * intr1.y)) : Vector3.zero;
				}
			...
			private static int QueryToLine(ref Vector2 test, ref Vector2 vec0, ref Vector2 vec1)
			{
				...
				return (det > Mathfex.ZeroTolerance ? +1 : (det < -Mathfex.ZeroTolerance ? -1 : 0));
			}
			...
			private static bool ContainsPoint(ref Triangle3 triangle, ref Plane3 plane, ref Vector3 point,
				out Triangle3Triangle3Intr info)
			{
				...
				Vector3 U0, U1;
				Vector3ex.CreateOrthonormalBasis(out U0, out U1, ref plane.Normal);
				...
				Vector2 ProjP = new Vector2(U0.Dot(PmV0), U1.Dot(PmV0));

				// The planar representation of the triangle <V0-V0,V1-V0,V2-V0>.
				Vector2 ProjV0 = Vector2.zero;
				Vector2 ProjV1 = new Vector2(U0.Dot(V1mV0), U1.Dot(V1mV0));
				Vector2 ProjV2 = new Vector2(U0.Dot(V2mV0), U1.Dot(V2mV0));
			...
			private static bool GetCoplanarIntersection(ref Plane3 plane, ref Triangle3 tri0, ref Triangle3 tri1, out Triangle3Triangle3Intr info)
			{
				...
				Vector2 save;
				Vector2 edge0 = projTri0.V1 - projTri0.V0;
				Vector2 edge1 = projTri0.V2 - projTri0.V0;
				...
				if (maxNormal == 0)
				{
					float invNX = 1.0f / plane.Normal.x;
					for (int i = 0; i < quantity; ++i)
					{
						info[i] = new Vector3(invNX * (plane.Constant - plane.Normal.y * intr[i].x - plane.Normal.z * intr[i].y), intr[i].x, intr[i].y);
					}
				}
				else if (maxNormal == 1)
				{
					float invNY = 1.0f / plane.Normal.y;
					for (int i = 0; i < quantity; ++i)
					{
						info[i] = new Vector3(intr[i].x, invNY * (plane.Constant - plane.Normal.x * intr[i].x - plane.Normal.z * intr[i].y), intr[i].y);
					}
				}
				else
				{
					float invNZ = 1.0f / plane.Normal.z;
					for (int i = 0; i < quantity; ++i)
					{
						info[i] = new Vector3(intr[i].x, intr[i].y, invNZ * (plane.Constant - plane.Normal.x * intr[i].x - plane.Normal.y * intr[i].y));
					}
				}
			...
			public static bool TestTriangle3Triangle3(ref Triangle3 triangle0, ref Triangle3 triangle1, out IntersectionTypes intersectionType)
			{
				...
				Vector3 E00 = triangle0.V1 - triangle0.V0;
				Vector3 E01 = triangle0.V2 - triangle0.V1;
				Vector3 E02 = triangle0.V0 - triangle0.V2;

				// Get normal vector of triangle0.
				Vector3 N0 = E00.UnitCross(E01);
				...
				Vector3 E10 = triangle1.V1 - triangle1.V0;
				Vector3 E11 = triangle1.V2 - triangle1.V1;
				Vector3 E12 = triangle1.V0 - triangle1.V2;

				// Get normal vector of triangle1.
				Vector3 N1 = E10.UnitCross(E11);
				...
				Vector3 N0xN1 = N0.UnitCross(N1);
				if (N0xN1.Dot(N0xN1) >= Mathfex.ZeroTolerance)
				{

	...
	namespace Math
	{
		...
		public static partial class Intersection
		{
			...
			private static void ClipConvexPolygonAgainstLine(ref Vector2 edgeStart, ref Vector2 edgeEnd, ref int quantity, ref Triangle2Triangle2Intr info)
			{
				...
				Vector2 edgeNormal = new Vector2(edgeStart.y - edgeEnd.y, edgeEnd.x - edgeStart.x);
				float c = edgeNormal.Dot(edgeStart);
				...
				{
					float distEntry = edgeNormal.Dot(info[i]) - c;
					if (distEntry > Mathfex.ZeroTolerance)
					{
						++positive;
						if (pIndex < 0) pIndex = i;
					}
					else if (distEntry < -Mathfex.ZeroTolerance)
					{
						++negative;
					}
					else
				...
				if (positive > 0)
				{
					if (negative > 0)
					{
						...
						if (pIndex > 0)
						{
							...
							float t = dist[cur] / (dist[cur] - dist[prv]);
				...
				else
				{
					...
					{
						int maxNormal = Mathf.Abs(edgeNormal.y) > Mathf.Abs(edgeNormal.x) ? 1 : 0;
						...
						if (q > 0)
						{
							...
							if (maxNormal == 0)
							{
								info.Point0 = new Vector2((c - edgeNormal.y * w0) / edgeNormal.x, w0);
								if (q == 2)
								{
									info.Point1 = new Vector2((c - edgeNormal.y * w1) / edgeNormal.x, w1);
								}
							}
							else
							{
								info.Point0 = new Vector2(w0, (c - edgeNormal.x * w0) / edgeNormal.y);
								if (q == 2)
								{
									info.Point1 = new Vector2(w1, (c - edgeNormal.x * w1) / edgeNormal.y);
								}
							}

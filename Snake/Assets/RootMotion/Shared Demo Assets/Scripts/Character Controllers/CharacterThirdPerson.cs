...
namespace RootMotion.Demos
{
    ...
    public class CharacterThirdPerson : CharacterBase
    {
        ...
        private void MoveFixed(Vector3 deltaPosition)
        {
            ...
            forwardMlp = 1;
            //float dotValUp = Vector3.Dot(normal, transform.up);
            //float dotForward = Vector3.Dot(normal, transform.forward);
            //forwardMlp = antiGravity ? 1 : Mathf.MoveTowards(forwardMlp, 1 + (1 - Mathf.Clamp(dotValUp, 0.8f, 1)) * Mathf.Sign(dotForward),Time.deltaTime*10);
            //Debug.Log(forwardMlp);
        }
        ...
        private void Raycast()
        {
            ...
            bool h = Physics.SphereCast(ray, spherecastRadius, out hit, airborneThreshold * 2f, groundLayers);

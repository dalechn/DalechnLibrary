...
namespace Dalechn
{
    public  partial class TansformTest:MonoBehaviour
    {
        ...
        private void GetDir()
        {
            ...
            // 看向摄像机的方向*输入的方向
            //方法1
            //rotateDir = cam.transform.TransformDirection(inputDir);
        ...
        // cos(θ+α) = cosθ*cosθ-sinα*sinα   cos(θ-α) = cosθ*cosθ+sinα*sinα
        ...
        {
            {
                ...
                float longitude = Mathf.Atan2(newDirection.x, newDirection.z) * Mathf.Rad2Deg; // tan-θ
                float latitude = -Mathf.Asin(newDirection.y / newDirection.magnitude) * Mathf.Rad2Deg; //sin-θ

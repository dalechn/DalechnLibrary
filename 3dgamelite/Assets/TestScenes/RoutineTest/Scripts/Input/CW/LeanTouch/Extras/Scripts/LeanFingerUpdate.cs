...
namespace Lean.Touch
{
    ...
    public class LeanFingerUpdate : MonoBehaviour
    {
        ...
        public Vector2Event OnEdgeMove { get { if (onEdgeMove == null) onEdgeMove = new Vector2Event(); return onEdgeMove; } }
        [SerializeField] private Vector2Event onEdgeMove;
        ...
        private void HandleFingerUpdate(LeanFinger finger)
        {
            ...
            if (finger.Index == LeanTouch.HOVER_FINGER_INDEX)
            {
                if (LeanTouch.Fingers.Count > 0)
                {
                    ...
                    {
                        var f = LeanTouch.Fingers[i];
                        //有手指点击就return;
                        if (f.Set&&f.Index != LeanTouch.HOVER_FINGER_INDEX)
                        {
                            return;
                        }
                    }
                ...
                var camera = CwHelper.GetCamera(ScreenDepth.Camera);
                Vector3 viewportPoint = camera.ScreenToViewportPoint(CwInput.GetMousePosition());

                float moveSpeedY = 0, moveSpeedX = 0;
                if (viewportPoint.y >= 0.98f && viewportPoint.y <= 1.2f) moveSpeedY = 1;
                else if (viewportPoint.y <= 0.02f && viewportPoint.y >= -0.2f) moveSpeedY = -1;
                if (viewportPoint.x <= 0.02f && viewportPoint.x >= -0.2f) moveSpeedX = -1;
                else if (viewportPoint.x >= 0.98f && viewportPoint.x <= 1.2f) moveSpeedX = 1;

                if (onEdgeMove != null)
                {
                    onEdgeMove.Invoke(new Vector2(moveSpeedX, moveSpeedY));
                }
            }
        }
    }
...
namespace Lean.Touch.Editor
{
    ...
    public class LeanFingerUpdate_Editor : CwEditor
    {
        protected override void OnInspector()
        {
            ...
            var usedH = Any(tgts, t => t.OnEdgeMove.GetPersistentEventCount() > 0);

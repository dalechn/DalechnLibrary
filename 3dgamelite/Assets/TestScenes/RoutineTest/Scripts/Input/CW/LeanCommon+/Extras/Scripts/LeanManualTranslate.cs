    ...
    public class LeanManualTranslate : MonoBehaviour
    {
        ...
        public void Translate(Vector3 vector)
        {
            ...
            if (plane && vector != Vector3.zero)
            {
                ...
                //和normal平行时 旋转90度
                if (Mathf.Abs(Vector3.Dot(vector, normal)) >=0.99999f)
                {
                    //tangent = Quaternion.AngleAxis(-90, finalTransform.right) * vector;
                    tangent = Quaternion.FromToRotation(finalTransform.up, normal) * vector;
                }
        ...
        private void UpdatePosition(float factor)
        {
            ...
            if (plane)
            {
                finalTransform.position = plane.GetClosest(finalTransform.position, offset);
            }
        ...
        private void Start()
        {
            ...
            offset = Vector3.Distance(plane.transform.position, finalTransform.position);
        }
    }
}

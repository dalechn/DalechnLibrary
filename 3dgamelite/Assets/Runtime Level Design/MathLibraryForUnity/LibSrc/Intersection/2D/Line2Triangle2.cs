...
namespace Dest
{
	namespace Math
	{
		...
		{
			private static void TriangleLineRelations(ref Vector2 origin  , ref Vector2 direction, ref Triangle2 triangle,
				...
				out int   positive, out int   negative, out int   zero)
			{
				...
				Vector2 diff = triangle.V0 - origin;
			...
			private static bool GetInterval(ref Vector2 origin, ref Vector2 direction, ref Triangle2 triangle,
				float     dist0, float dist1, float dist2,
				int       sign0, int   sign1, int   sign2,
				out float param0, out float param1)
			{
				// Project triangle onto line.
				Vector2 diff = triangle.V0 - origin;
				float proj0 = direction.Dot(diff);
				
				diff = triangle.V1 - origin;
				float proj1 = direction.Dot(diff);
				
				diff = triangle.V2 - origin;
				float proj2 = direction.Dot(diff);
			...
			public static bool TestLine2Triangle2(ref Line2 line, ref Triangle2 triangle, out IntersectionTypes intersectionType)
			{
				...
				else
				{
					...
					{
						float w0, w1;
						int quantity = Intersection.FindSegment1Segment1(param0, param1, float.NegativeInfinity, float.PositiveInfinity, out w0, out w1);
			...
			public static bool FindLine2Triangle2(ref Line2 line, ref Triangle2 triangle, out Line2Triangle2Intr info)
			{
				...
				else
				{
					...
					{
						float w0, w1;
						info.Quantity = Intersection.FindSegment1Segment1(param0, param1, float.NegativeInfinity, float.PositiveInfinity, out w0, out w1);

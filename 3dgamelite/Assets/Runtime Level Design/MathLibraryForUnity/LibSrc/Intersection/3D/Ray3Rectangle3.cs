...
namespace Dest
{
	namespace Math
	{
		...
		public static partial class Intersection
		{
			...
			{
				float DdN = ray.Direction.Dot(rectangle.Normal);
				if (Mathf.Abs(DdN) > _dotThreshold)
				{
					float signedDistance = rectangle.Normal.Dot(ray.Center - rectangle.Center);
					...
					if (parameter >= -_intervalThreshold)
					{
						...
						Vector3 point = ray.Center + parameter * ray.Direction;

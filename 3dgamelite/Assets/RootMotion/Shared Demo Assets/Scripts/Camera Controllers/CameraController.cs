...
namespace RootMotion
{
    ...
    public class CameraController : MonoBehaviour
    {
        ...
        public float zoomDamping = 10f; // The speed of interpolating the distance
        ...
        protected virtual void Awake()
        {
            ...
            currentYaw = x = angles.y;
            currentPitch = y = angles.x;
        ...
        public void UpdateMouse(Vector2 delta)
        {
#if MOBILE_INPUT
            x += delta.x;
            y = ClampAngle(y - delta.y, yMinLimit, yMaxLimit);
#endif
        }

        public void UpdateMouseScroll(float value)
        {
#if MOBILE_INPUT
            distanceTarget = Mathf.Clamp(distanceTarget * value, minDistance, maxDistance);
#endif
        }
        ...
        public void UpdateTransform(float deltaTime)
        {
            ...
            var factor = CW.Common.CwHelper.DampenFactor(rotationDamping, Time.deltaTime);

            currentPitch = Mathf.Lerp(currentPitch, y, factor);
            currentYaw = Mathf.Lerp(currentYaw, x, factor);

            // Rotation
            rotation = Quaternion.AngleAxis(currentYaw, Vector3.up) * Quaternion.AngleAxis(currentPitch, Vector3.right);
            ...
            {
                // Distance
                var zoomFactor = CW.Common.CwHelper.DampenFactor(zoomDamping, Time.deltaTime);

                distance += (distanceTarget - distance) * zoomFactor;
                ...
                //if (blockingLayers != -1)
                {
                    ...
                    if (Physics.SphereCast(t - f * blockingOriginOffset, blockingRadius, f, out hit, blockingOriginOffset + distanceTarget - blockingRadius, blockingLayers))
                    {
                        ...
                        //blockedDistance = Mathf.SmoothStep(blockedDistance, hit.distance + blockingRadius * (1f - blockedOffset) - blockingOriginOffset, blockingSmoothTime);
                    }

    ...
    public class vTriggerSoundByState : StateMachineBehaviour
    {
        ...
        void TriggerSound(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
        {
            ...
            if(audioSource)
            {
                ...
                if (PoolManager.Pools.ContainsKey(PoolNames.FootPool))
                {
                    source = PoolManager.Pools[PoolNames.FootPool].Spawn(audioSource.GetComponent<AudioSource>(), animator.transform.position, Quaternion.identity);
                }
                else
                {

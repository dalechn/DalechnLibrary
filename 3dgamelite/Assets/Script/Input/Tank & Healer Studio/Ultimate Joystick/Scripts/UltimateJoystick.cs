...
#endif
{
    ...
    //public UnityEventVector2 updateEvent;
	...
	void Update ()
	{
		...
		if(Application.isPlaying)
        {
            //updateEvent?.Invoke(new Vector2(GetHorizontalAxis(joystickName), GetVerticalAxis(joystickName)));

#if MOBILE_INPUT
            if(horizontalInput.useInput&&verticalInput.useInput)
            {
                CrossPlatformInputManager.SetAxis(horizontalInput.buttonName, GetHorizontalAxis(joystickName));
                CrossPlatformInputManager.SetAxis(verticalInput.buttonName, GetVerticalAxis(joystickName));
            }
#endif

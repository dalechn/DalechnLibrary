...
#endif
{
    ...
    protected RectTransform baseTrans;
    protected Rect buttonRect;
    protected CanvasGroup canvasGroup;
    protected bool getButtonDown = false;
    protected bool getButton = false;
    protected bool getButtonUp = false;
    protected bool getTapCount = false;
    ...
    protected Vector2 relativeTransformSize = Vector2.zero;
    protected Vector3 relativeTransformPos = Vector3.zero;
#if ENABLE_INPUT_SYSTEM
    ...
    {
        SetActive, SetIconActive, SetIconActiveAni, SetAlpha, SetGray
    }
    ...
    protected virtual void Start()
    {
        ...
        // 新增.关闭无用raycast
        if (buttonBase) buttonBase.raycastTarget = false;
        if (buttonIcon) buttonIcon.raycastTarget = false;
        if (tensionAccent) tensionAccent.raycastTarget = false;
        if (buttonHighlight) buttonHighlight.raycastTarget = false;
        if (buttonCooldown) buttonCooldown.raycastTarget = false;
    }
    ...
    public bool IsInRange(Vector2 inputPosition)
    {
    ...
    {
        if (!CrossPlatformInputManager.AxisExists(buttonName))
        {
            m_Axis = new CrossPlatformInputManager.VirtualAxis(buttonName);
            CrossPlatformInputManager.RegisterVirtualAxis(m_Axis);
        }
        else
        {
            m_Axis = CrossPlatformInputManager.VirtualAxisReference(buttonName);
        }
    }
    ...
    protected virtual void Update()
    {
        ...
        if (Application.isPlaying)
        {
            if (regist)
            {
                m_Axis.Update(Mathf.MoveTowards(m_Axis.GetValue, realValue, sensitivity * Time.deltaTime));
            }
        }
    }
    ...
    protected virtual void LateUpdate()
    {
        ...
        if (getButton && OnGetButton != null && !disabled)
            OnGetButton.Invoke();
    ...
    /// </summary>
    public void DisableButton(DisableMode mode = DisableMode.SetActive)
    {
        ...
        switch (mode)
        {
            case DisableMode.SetActive:
                ...
                break;
            case DisableMode.SetIconActive:
            case DisableMode.SetIconActiveAni:
                buttonIcon.enabled = false;
                break;
            case DisableMode.SetAlpha:
                canvasGroup.alpha = Mathf.Min(fadeTouched, fadeUntouched);
                break;
            case DisableMode.SetGray:
                // 使用前更换材质,有mask情况下失效
                buttonIcon.material.SetInt("_ShowGray", Convert.ToInt32(true));
                break;
        }
    ...
    /// </summary>
    public void EnableButton(DisableMode mode = DisableMode.SetActive)
    {
        // 修改
        disabled = false;
        ...
        switch (mode)
        {
            ...
            case DisableMode.SetIconActive:
                ...
                break;
            case DisableMode.SetIconActiveAni:
                if (!buttonIcon.enabled)
                {
                    buttonIcon.enabled = true;
                    CJTools.GameUtils.DampAnimation(gameObject);
                }
                break;
            case DisableMode.SetAlpha:
                canvasGroup.alpha = Mathf.Max(fadeTouched, fadeUntouched);
                break;
            case DisableMode.SetGray:
                // 使用前更换材质,有mask情况下失效
                buttonIcon.material.SetInt("_ShowGray", Convert.ToInt32(false));
                break;
    ...
    public static void DisableButton(string buttonName, DisableMode mode = DisableMode.SetActive)
    {
        // If this button name is not connected with an Ultimate Button component, then return.
        if (!ButtonConfirmed(buttonName))
            return;

        UltimateButtons[buttonName].DisableButton(mode);
    }
    ...
    /// </summary>
    /// <param name="buttonName">The name of the desired Ultimate Button.</param>
    public static void EnableButton(string buttonName, DisableMode mode = DisableMode.SetActive)
    {
        // If this button name is not connected with an Ultimate Button component, then return.
        if (!ButtonConfirmed(buttonName))
            return;

        UltimateButtons[buttonName].EnableButton(mode);
    }
    ...
    /// </summary>
    /// <param name="buttonName">The name of the desired Ultimate Button.</param>
    public static void ResetCooldown(string buttonName, DisableMode mode = DisableMode.SetActive)
    {
        // If this button name is not connected with an Ultimate Button component, then return.
        if (!ButtonConfirmed(buttonName))
            return;

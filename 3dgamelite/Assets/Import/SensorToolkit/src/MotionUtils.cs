...
namespace Micosmo.SensorToolkit {
    ...
    public static class MotionUtils {
        ...
        public static Vector3 SeekAngularAccel(float cSpring, float cDamp, ReferenceFrame frame, Vector3 angularVelocity, Vector3 tdir, Vector3 tup) {
            ...
            return SeekAccel(cSpring, cDamp, new Vector3(dPitch, dYaw, dRoll), new Vector3(vPitch, vYaw, vRoll));
        }

        public static Vector3 SeekPlanarAngularAccel(float cSpring, float cDamp, ReferenceFrame frame, Vector3 angularVelocity, Vector3 tdir, Vector3 planeNormal) {
        ...
        public static float SeekAngularAccel2D(float cSpring, float cDamp, Vector2 cdir, float angularVelocity, Vector2 tdir) {
            var dAngle = Vector2.SignedAngle(tdir, cdir);
            return SeekAccel(cSpring, cDamp, dAngle, angularVelocity);
        }

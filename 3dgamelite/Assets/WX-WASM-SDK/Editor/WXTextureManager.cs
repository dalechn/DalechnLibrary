...
namespace WeChatWASM
{
    ...
    public static class WXTextureManager
    {
        ...
        public static void WriteResult(WXEditorScriptObject conf) {
    ...
    androidETC2FallbackOverride: 0"
        ...
        public static void FindTexture()
        {
            ...
            foreach (string guid in list)
            {
                ...
                if (texture.width%4 !=0 || texture.height%4 != 0) {
                    ...
                    UnityEngine.Debug.LogWarning(path + " 宽高需要是4的倍数才能被替换为压缩纹理！");
                ...
                if (!isBad)
                {
                    ...
                    if (!isTextureAtlas) {
                        ...
                        if (sprite!= null && sprite.border!=null && (sprite.border.w !=0 || sprite.border.x !=0 || sprite.border.y !=0 || sprite.border.z != 0))
                        {
                            isTextureAtlas = true;
                        }
                    }
        ...
        public static void CompressImageCallback(string pngPath, string filePrefix, int width, int height, bool needMin)
        {
            ...
            if (!miniGameConf.CompressTexture.OnlyAstc)
            {
                ...
                if (miniGameConf.CompressTexture.useDXT5 && !File.Exists(filePrefix + ".dds.txt") && width%4==0 && height%4==0)
                {
                    PicCompressor.CompressDXT5(pngPath, filePrefix + ".dds");
                }

                if (width == height && !File.Exists(filePrefix + ".pvr.txt") && System.Array.IndexOf(potList,width) > -1)
                {

	...
	namespace Math
	{
		internal class ConvexHull1
		{
			...
			public static void Create(Real[] vertices, Real epsilon, out int dimension, out int[] indices)
			{
				...
				System.Array.Sort<SortedVertex>(sortedArray, (e1, e2) => Comparer<float>.Default.Compare(e1.Value, e2.Value));
		...
		internal class ConvexHull2
		{
			...
			public static bool Create(IList<Vector2> vertices, Real epsilon, out int dimension, out int[] indices)
			{
				Vector2ex.Information info = Vector2ex.GetInformation(vertices, epsilon);
				if (info == null)
				{
					dimension = -1;
					indices = null;
				...
				if (info.Dimension == 1)
				{
					...
					Vector2 direction = info.Direction[0];
					for (int i = 0; i < numVertices; ++i)
					{
				...
				dimension = 2;
				Vector2[] verticesCopy = new Vector2[numVertices];
								
				// Transform the vertices to the square [0,1]^2.
				Vector2 minValue = info.Min;
		...
		internal class ConvexHull3
		{
			...
			public static bool Create(IList<Vector3> vertices, Real epsilon, out int dimension, out int[] indices)
			{
				Vector3ex.Information info = Vector3ex.GetInformation(vertices, epsilon);
				if (info == null)
				{
					dimension = -1;
					indices = null;
				...
				if (info.Dimension == 1)
				{
					...
					Vector3 direction = info.Direction[0];
					for (int i = 0; i < numVertices; ++i)
					{
						Vector3 diff = vertices[i] - origin;
						projection[i] = direction.Dot(diff);
					}
					ConvexHull1.Create(projection, epsilon, out dimension, out indices);
					return true;
				...
				if (info.Dimension == 2)
				{
					Vector2[] projection = new Vector2[numVertices];
					Vector3 origin = info.Origin;
					Vector3 direction0 = info.Direction[0];
					Vector3 direction1 = info.Direction[1];
					for (int i = 0; i < numVertices; ++i)
					{
						Vector3 diff = vertices[i] - origin;
						projection[i] = new Vector2(direction0.Dot(diff), direction1.Dot(diff));
					}
					return ConvexHull2.Create(projection, epsilon, out dimension, out indices);
				}

				dimension = 3;
				Vector3[] verticesCopy = new Vector3[numVertices];

				// Transform the vertices to the cube [0,1]^3.
				Vector3 minValue = info.Min;
				Real scale = ((Real)1) / info.MaxRange;
				for (int i = 0; i < numVertices; ++i)
				{
					verticesCopy[i] = (vertices[i] - minValue) * scale;
		...
		public static class ConvexHull
		{
			...
			public static bool Create2D(IList<Vector2> points, out int[] indices, out int dimension, float epsilon = Mathfex.ZeroTolerance)
			{

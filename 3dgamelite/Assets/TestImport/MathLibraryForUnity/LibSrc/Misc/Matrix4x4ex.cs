	...
	namespace Math
	{
		public static class Matrix4x4ex
		{
			...
			public static void CreateRotationAngleUnitAxis(float angleInDegrees, Vector3 normalizedAxis, out Matrix4x4 matrix)
			{
				...
				float sin = Mathf.Sin(angle);
				float oneMinusCos = 1f - cos;
				float x2 = normalizedAxis.x * normalizedAxis.x;
				float y2 = normalizedAxis.y * normalizedAxis.y;
				float z2 = normalizedAxis.z * normalizedAxis.z;
				float xym = normalizedAxis.x * normalizedAxis.y * oneMinusCos;
				float xzm = normalizedAxis.x * normalizedAxis.z * oneMinusCos;
				float yzm = normalizedAxis.y * normalizedAxis.z * oneMinusCos;
				float xSin = normalizedAxis.x * sin;
				float ySin = normalizedAxis.y * sin;
				float zSin = normalizedAxis.z * sin;
			...
			{
				float a0 = matrix.m00 * matrix.m11 - matrix.m10 * matrix.m01;
				float a1 = matrix.m00 * matrix.m21 - matrix.m20 * matrix.m01;
				float a2 = matrix.m00 * matrix.m31 - matrix.m30 * matrix.m01;
				float a3 = matrix.m10 * matrix.m21 - matrix.m20 * matrix.m11;
				float a4 = matrix.m10 * matrix.m31 - matrix.m30 * matrix.m11;
				float a5 = matrix.m20 * matrix.m31 - matrix.m30 * matrix.m21;
				float b0 = matrix.m02 * matrix.m13 - matrix.m12 * matrix.m03;
				float b1 = matrix.m02 * matrix.m23 - matrix.m22 * matrix.m03;
				float b2 = matrix.m02 * matrix.m33 - matrix.m32 * matrix.m03;
				float b3 = matrix.m12 * matrix.m23 - matrix.m22 * matrix.m13;
				float b4 = matrix.m12 * matrix.m33 - matrix.m32 * matrix.m13;
				float b5 = matrix.m22 * matrix.m33 - matrix.m32 * matrix.m23;
			...
			public static void Inverse(ref Matrix4x4 matrix, float epsilon = Mathfex.ZeroTolerance)
			{
				float a0 = matrix.m00 * matrix.m11 - matrix.m10 * matrix.m01;
				float a1 = matrix.m00 * matrix.m21 - matrix.m20 * matrix.m01;
				float a2 = matrix.m00 * matrix.m31 - matrix.m30 * matrix.m01;
			...
			/// <param name="epsilon">Small positive number used to compare determinant with zero</param>
			public static void Inverse(ref Matrix4x4 matrix, out Matrix4x4 inverse, float epsilon = Mathfex.ZeroTolerance)
			{
				float a0 = matrix.m00 * matrix.m11 - matrix.m10 * matrix.m01;
				float a1 = matrix.m00 * matrix.m21 - matrix.m20 * matrix.m01;
				float a2 = matrix.m00 * matrix.m31 - matrix.m30 * matrix.m01;
			...
			{
				Vector3 N     = shadowPlane.Normal;
				float   d     = shadowPlane.Constant;
				float   NdotL = N.x * dirLightOppositeDirection.x + N.y * dirLightOppositeDirection.y + N.z * dirLightOppositeDirection.z;
			...
			public static void CreateShadowPoint(Plane3 shadowPlane, Vector3 pointLightPosition, out Matrix4x4 result)
			{
				Vector3 N     = shadowPlane.Normal;
				float   d     = shadowPlane.Constant;
				float   NdotL = N.x * pointLightPosition.x + N.y * pointLightPosition.y + N.z * pointLightPosition.z;
			...
			public static void CreateShadow(Plane3 shadowPlane, Vector4 lightData, out Matrix4x4 result)
			{
				Vector3 N     = shadowPlane.Normal;
				float   d     = shadowPlane.Constant;
				float   NdotL = N.x * lightData.x + N.y * lightData.y + N.z * lightData.z;

	...
	namespace Math
	{
		...
		public static partial class Intersection
		{
			...
			public static bool TestPlane3Triangle3(ref Plane3 plane, ref Triangle3 triangle)
			{
				...
				float SD0 = plane.SignedDistanceTo(ref triangle.V0);
				if (Mathf.Abs(SD0) <= _distanceThreshold)
				{
					SD0 = zero;
				}

				float SD1 = plane.SignedDistanceTo(ref triangle.V1);
				if (Mathf.Abs(SD1) <= _distanceThreshold)
				{
					SD1 = zero;
				}

				float SD2 = plane.SignedDistanceTo(ref triangle.V2);
				if (Mathf.Abs(SD2) <= _distanceThreshold)
				{
					SD2 = zero;
				}

...
namespace Dest
{
	namespace Math
	{
		...
		public static partial class Intersection
		{
			...
			{
				Vector2 diff = circle0.Center - circle1.Center;
				float rSum = circle0.Radius + circle1.Radius;
			...
			public static bool FindCircle2Circle2(ref Circle2 circle0, ref Circle2 circle1, out Circle2Circle2Intr info)
			{
				...
				Vector2 U = circle1.Center - circle0.Center;
				float USqrLen = U.sqrMagnitude;
				float R0 = circle0.Radius, R1 = circle1.Radius;
				float R0mR1 = R0 - R1;

				if (USqrLen < Mathfex.ZeroToleranceSqr &&
					Mathf.Abs(R0mR1) < Mathfex.ZeroTolerance)
				{
				...
				if (USqrLen < R0pR1Sqr)
				{
					if (R0mR1Sqr < USqrLen)
					{
						float invUSqrLen = 1f / USqrLen;
						float s = 0.5f * ((R0 * R0 - R1 * R1) * invUSqrLen + 1f);
						Vector2 tmp = circle0.Center + s * U;

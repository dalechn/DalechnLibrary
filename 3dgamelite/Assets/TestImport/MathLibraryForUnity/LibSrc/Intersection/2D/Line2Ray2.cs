	...
	namespace Math
	{
		...
		public static partial class Intersection
		{
			private static IntersectionTypes Classify(ref Line2 line, ref Ray2 ray, out float s0, out float s1)
			{
				...
				Vector2 originDiff = ray.Center - line.Center;

				s0 = s1 = 0f;

				float D0DotPerpD1 = line.Direction.DotPerp(ray.Direction);
				...
				if (Mathf.Abs(D0DotPerpD1) > _dotThreshold)
				{
					...
					float diffDotPerpD1 = originDiff.DotPerp(ray.Direction);
				...
				float diffNDotPerpD1 = originDiff.DotPerp(ray.Direction);

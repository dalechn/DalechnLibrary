	...
	namespace Math
	{
		...
		public struct Box3
		{
			...
			public Box3(ref AAB3 box)
			{
				...
				Axis0 = Vector3ex.UnitX;
				Axis1 = Vector3ex.UnitY;
				Axis2 = Vector3ex.UnitZ;
			}
			...
			public static Box3 CreateFromPoints(IList<Vector3> points)
			{
				...
				Vector3 diff = points[0] - box.Center;
				Vector3 pmin = new Vector3(diff.Dot(box.Axis0), diff.Dot(box.Axis1), diff.Dot(box.Axis2));
				Vector3 pmax = pmin;
			...
			public void CalcVertices(
				out Vector3 vertex0, out Vector3 vertex1, out Vector3 vertex2, out Vector3 vertex3,
				out Vector3 vertex4, out Vector3 vertex5, out Vector3 vertex6, out Vector3 vertex7)
			{
				Vector3 extAxis0 = Extents.x * Axis0;
				Vector3 extAxis1 = Extents.y * Axis1;
				Vector3 extAxis2 = Extents.z * Axis2;
			...
			public void Include(ref Box3 box)
			{
				...
				float invLength = Mathfex.InvSqrt(Quaternion.Dot(q, q));
				q.x *= invLength;
				q.y *= invLength;
				q.z *= invLength;
				q.w *= invLength;
				...
				float dot;
				Vector3 diff;
				Vector3 pmin = Vector3ex.Zero;
				Vector3 pmax = Vector3ex.Zero;
				Vector3[] vertex = this.CalcVertices();

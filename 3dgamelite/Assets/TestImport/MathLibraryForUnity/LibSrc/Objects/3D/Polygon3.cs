...
namespace Dest
{
	namespace Math
	{
		...
		public class Polygon3
		{
			...
			/// </summary>
			public Vector3 this[int vertexIndex] { get { return _vertices[vertexIndex]; } set { _vertices[vertexIndex] = value; } }
			...
			{
				float signedDistance = _plane.Normal.Dot(vertex) - _plane.Constant;
			...
			public void ProjectVertices()
			{
				...
				{
					float signedDistance = _plane.Normal.Dot(_vertices[i]) - _plane.Constant;
			...
			public void UpdateEdges()
			{
				...
				for (int i0 = vertexCount - 1, i1 = 0; i1 < vertexCount; i0 = i1, ++i1)
				{
					...
					_edges[i0].Length = Vector3ex.Normalize(ref direction);
			...
			public void UpdateEdge(int edgeIndex)
			{
				...
				_edges[edgeIndex].Length = Vector3ex.Normalize(ref direction);
			...
			public bool HasZeroCorners(float threshold = Mathfex.ZeroTolerance)
			{
				...
				for (int i0 = edgesLength - 1, i1 = 0; i1 < edgesLength; i0 = i1, ++i1)
				{
					...
					float dot = Vector3.Dot(edge0, edge1);
					if (dot >= thr)
					{
						return true;
					}

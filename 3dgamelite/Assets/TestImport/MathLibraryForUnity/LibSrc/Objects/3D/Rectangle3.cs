	...
	namespace Math
	{
		...
		public struct Rectangle3
		{
			...
			public static Rectangle3 CreateFromCCWPoints(Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3)
			{
				...
				result.Extents.x = Vector3ex.Normalize(ref axis0) * .5f;
				result.Extents.y = Vector3ex.Normalize(ref axis1) * .5f;
				result.Axis0 = axis0;
				result.Axis1 = axis1;
			...
			{
				Vector3 extAxis0 = Axis0 * Extents.x;
				Vector3 extAxis1 = Axis1 * Extents.y;

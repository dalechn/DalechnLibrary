...
namespace Dest
{
	namespace Math
	{
		...
		public struct AAB2
		{
			...
			{
				return new AAB2() { Min = new Vector2(value.xMin, value.yMin), Max = new Vector2(value.xMax, value.yMax) };
			}
			...
			{
				vertex0 = Min;
				vertex1 = new Vector2(Max.x, Min.y);
				vertex2 = Max;
				vertex3 = new Vector2(Min.x, Max.y);
			}
			...
			{
				array[0] = Min;
				array[1] = new Vector2(Max.x, Min.y);
				array[2] = Max;
				array[3] = new Vector2(Min.x, Max.y);
			}

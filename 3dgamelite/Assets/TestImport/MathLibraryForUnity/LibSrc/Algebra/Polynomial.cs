...
namespace Dest
{
	namespace Math
	{
		...
		public class Polynomial
		{
			...
			public float this[int index] { get { return _coeffs[index]; } set { _coeffs[index] = value; } }
			...
			/// </summary>
			public void Compress(float epsilon = Mathfex.ZeroTolerance)
			{
				int tempDegree = (int)_degree;
				for (int i = tempDegree; i >= 0; --i)
				{
					if (Mathf.Abs(_coeffs[i]) <= epsilon)
					{
						--tempDegree;
					}
					else
				...
				{
					_degree = tempDegree;
					float invLeading = 1f / _coeffs[_degree];
					_coeffs[_degree] = 1f;
					for (int i = 0; i < _degree; ++i)
					{

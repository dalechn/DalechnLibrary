...
using Vector2Real = UnityEngine.Vector2;
...
namespace Dest
{
	namespace Math
	{
		internal class Query2 : Query
		{
			...
			private Vector2Real[] _vertices;

			public Query2(Vector2Real[] vertices)
			{
				_vertices = vertices;
			}
			...
			public int ToLine(ref Vector2Real test, int v0, int v1)
			{
				bool positive = Sort(ref v0, ref v1);

				Vector2Real vec0 = _vertices[v0];
				Vector2Real vec1 = _vertices[v1];
			...
			public int ToTriangle(ref Vector2Real test, int v0, int v1, int v2)
			{
			...
			public int ToCircumcircle(ref Vector2Real test, int v0, int v1, int v2)
			{
				bool positive = Sort(ref v0, ref v1, ref v2);

				Vector2Real vec0 = _vertices[v0];
				Vector2Real vec1 = _vertices[v1];
				Vector2Real vec2 = _vertices[v2];

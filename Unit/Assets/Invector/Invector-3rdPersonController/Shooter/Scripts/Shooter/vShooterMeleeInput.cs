...
namespace Invector.vCharacterController
{
    ...
    public class vShooterMeleeInput : vMeleeCombatInput, vIShooterIKController, PlayerController.vILockCamera
    {
        ...
        protected float lastAimDistance =100;
        ...
        protected override void Start()
        {
            ...
            {
                _controlAimCanvas = GetComponentInChildren<vControlAimCanvas>();
        ...
        protected virtual void UpdateShooterAnimations()
        {
            ...
            //if(CurrentActiveWeapon && IsAiming) animator.SetLayerWeight(shotLayer,  CurrentActiveWeapon.scopeShootAnimationWeight);
        ...
        protected virtual void UpdateAimPosition()
        {
            ...
            AimPosition = camT.position + camT.forward * 100;
            ...
            if (shooterManager.raycastAimTarget && CurrentActiveWeapon.raycastAimTarget)
            {
                ...
                float rayDist = cameraMain.farClipPlane;
                if (Physics.Raycast(ray, out hit, rayDist, shooterManager.damageLayer))
                {
                    if (hit.collider.transform.IsChildOf(transform))
                    {
                        var collider = hit.collider;
                        var hits = Physics.RaycastAll(ray, rayDist, shooterManager.damageLayer);
                        var dist = rayDist;
        ...
        protected virtual Vector3 targetArmAlignmentPosition
        {
            get
            {
                return isUsingScopeView && controlAimCanvas.scopeBackgroundCamera ?
                    cameraMain.transform.position + cameraMain.transform.forward * lastAimDistance : shooterManager.alignArmToHitPoint ?
                    AimPosition : cameraMain.transform.position + cameraMain.transform.forward * 100;
            }
        }
        ...
        protected virtual void UpdateAimHud()
        {
            ...
            if (controlAimCanvas.scopeBackgroundCamera && CurrentActiveWeapon.scopeTarget)
            {
                ...
                {
                    RaycastHit hit;
                    if (Physics.Raycast(CurrentActiveWeapon.scopeTarget.position, CurrentActiveWeapon.scopeTarget.forward, out hit, 100f, shooterManager.blockAimLayer)
                        || Physics.Raycast(CurrentActiveWeapon.scopeTarget.position, CurrentActiveWeapon.scopeTarget.forward, out hit, 100f, shooterManager.damageLayer))
                    {
                        AimPosition = hit.point;
                    }
                    ...
                    var scopeLookDirection = Vector3.Lerp(controlAimCanvas.scopeBackgroundCamera.transform.forward, CurrentActiveWeapon.scopeTarget.forward, shooterManager.isShooting ?
                        scopeDirectionWeight = 0 : scopeDirectionWeight = Mathf.Lerp(scopeDirectionWeight, 1.001f, (10f / CurrentActiveWeapon.shootFrequency) * Time.deltaTime));
    ...
    public static partial class vAnimatorParameters
    {
        ...
        public static int ReoladID = Animator.StringToHash("ReoladID");
        public static int CancelReload = Animator.StringToHash("CancelReload");
    }
}
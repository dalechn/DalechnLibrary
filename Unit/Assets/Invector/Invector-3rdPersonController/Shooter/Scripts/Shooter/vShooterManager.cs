...
namespace Invector.vShooter
{
    ...
    public class vShooterManager : vMonoBehaviour, IWeaponEquipmentListener
    {
        ...
        public virtual void ReloadWeapon()
        {
            ...
            if (weapon.ammoCount < weapon.clipSize && ((weapon.isInfinityAmmo||AllAmmoInfinity) || WeaponHasUnloadedAmmo()) && !weapon.dontUseReload)
            {
                ...
                if (animator)
                {
                    animator.SetInteger(vAnimatorParameters.ReloadID, GetReloadID());
                    animator.SetTrigger(vAnimatorParameters.Reload);
                }
                ...
                if (!cancelReload)
                {
                    ...
                    if (weapon.reloadOneByOne && weapon.ammoCount < weapon.clipSize && WeaponHasUnloadedAmmo())
                    {
                        ...
                        else
                        {
                            ...
                            if (!cancelReload)
                            {
                                animator.SetInteger(vAnimatorParameters.ReloadID, weapon.reloadID);
                                animator.SetTrigger(vAnimatorParameters.Reload);
        ...
        public virtual void Shoot(Vector3 aimPosition, bool applyHipfirePrecision = false)
        {
            ...
            if (applyRecoil)
            {
                ...
                var recoilUp = Random.Range(weapon.recoilUp/2, weapon.recoilUp);
        ...
        protected virtual IEnumerator Recoil(float horizontal, float up)
        {
            if (animator)
            {
                animator.SetTrigger(vAnimatorParameters.IsShoot);
            }
            //if (tpCamera != null&&applyRecoilToCamera)
            //{
            //    tpCamera.RotateCamera(horizontal, up);
            //}
            bl_EventHandler.DoCameraRecoil( horizontal, up);
        ...
        public virtual void CameraSway()
        {
            ...
            bl_EventHandler.DoCameraSway(weapon.cameraStability);
        }
    }
    ...
    public static partial class vAnimatorParameters
    {
        ...
        /// </summary>
        public static int IsShoot = Animator.StringToHash("Shoot");
        /// <summary>
        ...
        /// </summary>
        public static int ReloadID = Animator.StringToHash("ReloadID");
    }
}
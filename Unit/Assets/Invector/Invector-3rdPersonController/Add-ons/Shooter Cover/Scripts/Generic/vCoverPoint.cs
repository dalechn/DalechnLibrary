...
namespace Invector.vCharacterController
{
    ...
    [vClassHeader("Cover Point", openClose = false)]
    public class vCoverPoint : vMonoBehaviour
    {
       
        [System.Serializable]
        public class CoverEvent : UnityEngine.Events.UnityEvent<vCoverPoint, GameObject> { };
        ...
        public vCoverPoint leftCorner, rightCorner;
        public vCoverPoint left, right;
        ...
        [SerializeField,HideInInspector] protected Vector3 lastPosition;
        [SerializeField, HideInInspector] float lastOffsetPosePositionX;
        [SerializeField, HideInInspector] float lastRayCastNeighborOffsetX;
        [SerializeField, HideInInspector] Vector3 lastColliderCenter;
        [SerializeField, HideInInspector] Vector3 lastColliderSize;

        private IEnumerator Start()
        {
            yield return new WaitForEndOfFrame();
            gameObject.SetActive(isValid = CheckPosePositionOnNavMesh());
        ...
        public virtual bool CheckPosePositionOnNavMesh()
        {
            NavMeshHit hit;
        ...
        {
            bool isRight = false;
            vCoverPoint _possibleRight = null;
            ...
            {
                _possibleRight = hit.transform.gameObject.GetComponent<vCoverPoint>();
            }
            else
            {
                for (int i = 0; i < 6; i++)
                {
                    ...
                    {
                        Debug.DrawRay(rayOrigin, dir * space, Color.green);
                    }
                }
                if (!_possibleRight)
                {
                    for (int i = 0; i < 6; i++)
                    {
                        ...
                        {
                            Debug.DrawRay(rayOrigin, dir * space, Color.yellow);
                        }
                    }
                }
            }
        ...
        {
            bool isLeft = false;
            vCoverPoint _possibleLeft = null;

            if (RayCastConnections(transform.position + transform.TransformDirection(boxCollider.center) - transform.right * boxCollider.size.x * rayCastNeighborOffsetX, transform.right, out hit, boxCollider.size.x * (1.1f + rayCastNeighborOffsetX), mask))
            {
                _possibleLeft = hit.transform.gameObject.GetComponent<vCoverPoint>();
            }
            else
            {
                for (int i = 0; i < 6; i++)
        ...
        private void OnDrawGizmos()
        {
            ...
            Gizmos.DrawSphere(posePosition, 0.1f);
            ...
            Gizmos.color = isValid&&boxCollider.enabled ? Color.white * 0.8f : Color.red * 0.8f;
            Gizmos.DrawWireCube(Vector3.zero, new Vector3(1, 1, 0.2f));

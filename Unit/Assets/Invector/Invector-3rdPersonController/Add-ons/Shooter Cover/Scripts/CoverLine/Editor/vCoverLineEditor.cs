        ...
        {           
            ...
            if (point.conectedLeft && point.curveType!= vCoverLine.CurveType.Linear && point.curveType!= vCoverLine.CurveType.LinearLeft)
            {
                ...
                if (e.shift || selectionState == SelectionState.Left && currentPoint == point)
                {
                    ...
                    if ((e.shift && !e.control))
                        tangentLeft = Handles.FreeMoveHandle(tangentLeft, Quaternion.identity, HandleUtility.GetHandleSize(tangentLeft) * 0.15f, Vector3.zero, Handles.RectangleHandleCap);
                    else
                    {
            ...
            if (point.conectedRight && point.curveType != vCoverLine.CurveType.Linear && point.curveType != vCoverLine.CurveType.LinearRight)
            {
                ...
                if ((e.shift && !e.control) || selectionState == SelectionState.Right && currentPoint == point)
                {
                    EditorGUI.BeginChangeCheck();
                    if ((e.shift && !e.control))
                        tangentRight = Handles.FreeMoveHandle(tangentRight, Quaternion.identity, HandleUtility.GetHandleSize(tangentRight) * 0.15f, Vector3.zero, Handles.RectangleHandleCap);
                    else
                    {
            ...
            if ((e.shift && !e.control)  || selectionState == SelectionState.Center && currentPoint == point)
            {
                ...
                if ((e.shift && !e.control))
                {
                    position = Handles.FreeMoveHandle(position, Quaternion.identity, HandleUtility.GetHandleSize(position) * 0.15f, Vector3.zero, Handles.RectangleHandleCap);
                }

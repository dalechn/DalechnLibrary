    ...
    public class vAICoverAreaEditor : vEditorBase
    {
        ...
        private void OnSceneGUI()
        {
            ...
            if (coverArea.coverLines.Count > 0)
            {
                ...
                if (canEdit && Handles.Button(coverArea.coverLines[0].p2.position, Quaternion.identity, 0.1f, 0.1f, Handles.SphereHandleCap))
                {
                ...
                for (int i = 0; i < coverArea.coverLines.Count; i++)
                {
                    ...
                    if (i > 0 && coverArea.coverLines[0].p1 != coverArea.coverLines[i].p2)
                        if (canEdit && Handles.Button(coverArea.coverLines[i].p2.position, Quaternion.identity, 0.1f, 0.1f, Handles.SphereHandleCap))
                        {
            ...
            if (canEdit && currentSelectedP && coverArea.coverLines.Count>0 && coverArea.coverLines[0].p1!= currentSelectedP)
            { 
            ...
            Handles.color = color;
            if (canEdit && coverArea.coverLines.Count > 1)
            {
        ...
        private void DrawCoverPoint(vAICoverArea.CoverLine coverLine, Vector3 right, float realWidth, Vector3 startP)
        {
            ...
            for (int i = 0; i < coverLine.coverPoints.Count; i++)
            {
                ...
                if(coverArea.drawTargetPosition)
                {
        ...
        public bool canEdit
        {
            get
            {
                GameObject prefab = PrefabUtility.GetOutermostPrefabInstanceRoot(coverArea.gameObject);
                return !prefab || UnityEditor.Experimental.SceneManagement.PrefabStageUtility.GetCurrentPrefabStage() != null;
            }
        }
        public override void OnInspectorGUI()
        {
            ...
            if (!canEdit)
            {               
              EditorGUILayout.HelpBox("This area is in Prefab Instance\n Unpack  or Open prefab to edit the cover points", MessageType.Error);               
            }
            else
            if (coverArea)
            {

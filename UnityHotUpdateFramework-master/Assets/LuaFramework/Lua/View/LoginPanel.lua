...
function LoginPanel.OnShow(obj)
    ...
    uiBinder:SetText(
        "versionText",
        string.format("app: %s res: %s", VersionMgr.instance.appVersion, VersionMgr.instance.resVersion)
    )
    -- 注册按钮
    uiBinder:SetBtnClick(
        "registBtn",
        function()
            if not this.CheckAccountPwd(accountInput.text, passwordInput.text) then
                return
            end
            LoginLogic.DoRegist(
                accountInput.text,

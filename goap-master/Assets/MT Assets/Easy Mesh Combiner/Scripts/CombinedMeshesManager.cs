    ...
    public class CombinedMeshesManager : MonoBehaviour
    {
        ...
        public class CustomInspector : UnityEditor.Editor
        {
            ...
            public override void OnInspectorGUI()
            {
                ...
                //Start of UI
                EditorGUILayout.HelpBox("This GameObject contains the meshes you previously combined. Through this component you can manage the mesh resulting from the merge.", UnityEditor.MessageType.Info);
                GUILayout.Space(10);
                ...
                EditorGUILayout.HelpBox(informationsBuild.ToString(), UnityEditor.MessageType.Info);
                ...
                if (warningsOfChecks.Count > 0)
                {
                    ...
                    if (script.hideWarningsForThisMerge == false)
                    {
                        ...
                        foreach (string str in warningsOfChecks)
                            EditorGUILayout.HelpBox(str, UnityEditor.MessageType.Warning);
                        EditorGUILayout.EndVertical();
                    }
                }
            }
            ...
            private void RenderTheBoxOfMaterialsAndAssetsManagement(CombinedMeshesManager script)
            {
                ...
                if (script.mergeMethodUsed == MergeMethod.OneMeshPerMaterial)
                {
                    ...
                    if (objects.Keys.Count == 0)
                        EditorGUILayout.HelpBox("Oops! The original materials of this merge were not found!", UnityEditor.MessageType.Info);
                    if (objects.Keys.Count > 0)

    ...
    public class MeshCombinerTool : EditorWindow
    {
        ...
        */
        ...
        public class LogOfMerge
        {
            ...
            public UnityEditor.MessageType logType;
            public string message;

            public LogOfMerge(UnityEditor.MessageType logType, string message)
            {
        ...
        public static void OpenWindow()
        {
            ...
            window.minSize = new Vector2(610, 660);
            window.maxSize = new Vector2(610, 660);
        ...
        void OnGUI()
        {
            ...
            {
                EditorGUILayout.HelpBox("Unable to load required files. Please reinstall Easy Mesh Combiner to correct this problem.", UnityEditor.MessageType.Error);
                return;
            }
            ...
            if (validsGameObjectsSelected.Count == 0 || isAllMergeParamsValids == false)
            {
                ...
                GUILayout.Space(12);
                EditorGUILayout.HelpBox("Cannot merge GameObjects and meshes. Check the Logs of Merge above to understand why.", UnityEditor.MessageType.Warning);
                GUILayout.Space(11);
        ...
        void UI_MergePreferences()
        {
            ...
            GUILayout.Space(20);
            scrollPosForPreferences = EditorGUILayout.BeginScrollView(scrollPosForPreferences, GUILayout.Width(293), GUILayout.Height(287));
        ...
        void UI_MergePreferences_AllInOne()
        {
            ...
            if (meshCombinerPreferences.allInOneParams.materialToUse == null)
                EditorGUILayout.HelpBox("Please add a custom material. This custom material will have its properties copied and will be associated with the merged mesh. This merge method cannot function without a material.", UnityEditor.MessageType.Error);
        ...
        void UI_LogsOfMerge()
        {
            ...
            scrollPosForLogs = EditorGUILayout.BeginScrollView(scrollPosForLogs, GUILayout.Width(293), GUILayout.Height(335));
        ...
        void UI_StatsBar(Texture arrowIcon, Texture arrowDoneIcon)
        {
            ...
            //Stats bar
            GUILayout.BeginHorizontal("box");
            scrollPosForStats = EditorGUILayout.BeginScrollView(scrollPosForStats, GUILayout.Width(596), GUILayout.Height(127));
        ...
        private void ValidateGameObjectsSelection()
        {
            ...
            {
                logsOfBeforeMerge.Add(new LogOfMerge(UnityEditor.MessageType.Info, "No GameObject has been selected. Select at least one GameObject so that Easy Mesh Combiner can work."));
                return;
            }
            ...
            for (int i = 0; i < gameObjectsWithMeshFound.Count; i++)
            {
                ...
                {
                    logsOfBeforeMerge.Add(new LogOfMerge(UnityEditor.MessageType.Error, "GameObject \"" + thisGameObject.name + "\" does not have the Mesh Filter component, so it is not a valid mesh and will be ignored in the merge process."));
                    canAddThisGameObjectToValidGameObjects = false;
                }
                //Verify if MeshRenderer is null
                if (thisMeshRenderer == null)
                {
                    logsOfBeforeMerge.Add(new LogOfMerge(UnityEditor.MessageType.Error, "GameObject \"" + thisGameObject.name + "\" does not have the Mesh Renderer component, so it is not a valid mesh and will be ignored in the merge process."));
                    canAddThisGameObjectToValidGameObjects = false;
                }
                //Verify if SharedMesh is null
                if (thisMeshRenderer != null && thisMeshFilter.sharedMesh == null)
                {
                    logsOfBeforeMerge.Add(new LogOfMerge(UnityEditor.MessageType.Error, "GameObject \"" + thisGameObject + "\" does not have a Mesh in Mesh Filter component, so it is not a valid mesh and will be ignored in the merge process."));
                    canAddThisGameObjectToValidGameObjects = false;
                }
                ...
                if (thisMeshFilter != null && thisMeshRenderer != null && thisMeshFilter.sharedMesh != null)
                    ...
                    {
                        logsOfBeforeMerge.Add(new LogOfMerge(UnityEditor.MessageType.Error, "The Mesh Renderer component found in GameObject \"" + thisGameObject.name + "\" has more or less material needed. The mesh that is in this GameObject has " + thisMeshFilter.sharedMesh.subMeshCount.ToString() + " submeshes, but has a number of " + thisMeshRenderer.sharedMaterials.Length.ToString() + " materials. This mesh will be ignored during the merge process."));
                        canAddThisGameObjectToValidGameObjects = false;
                    }
                ...
                if (thisMeshRenderer != null)
                {
                    for (int x = 0; x < thisMeshRenderer.sharedMaterials.Length; x++)
                        ...
                        {
                            logsOfBeforeMerge.Add(new LogOfMerge(UnityEditor.MessageType.Error, "Material " + x.ToString() + " in Mesh Renderer present in component \"" + thisGameObject.name + "\" is null. For the merge process to work well, all materials must be completed. This GameObject will be ignored in the merge process."));
                            canAddThisGameObjectToValidGameObjects = false;
                        }
                    ...
                    {
                        logsOfBeforeMerge.Add(new LogOfMerge(UnityEditor.MessageType.Warning, "The mesh in GameObject \"" + thisGameObject.name + "\" contains many vertices and a large amount of materials. Due to a Unity limitation, you may experience a longer time to combine meshes using Lightmap Support, or you may experience errors during the merge process. If this happens, try reducing the amount of submeshes present in this mesh. If no problem occurs, do not worry, everything went as expected."));
                ...
                {
                    logsOfBeforeMerge.Add(new LogOfMerge(UnityEditor.MessageType.Error, "GameObject \"" + thisGameObject.name + "\" is the result of a previous merge, so it will be ignored by this merge."));
                    canAddThisGameObjectToValidGameObjects = false;
                }
            ...
            if (isAllMergeParamsValids == false)
                logsOfBeforeMerge.Add(new LogOfMerge(UnityEditor.MessageType.Error, "There are invalid merge parameters in the \"Preferences of Merge\" box. Please check all parameters and make sure that they are valid so that it is possible to perform the merge."));
        ...
        public void DoCombineMeshes_OneMeshPerMaterial()
        {
            ...
            //Add the log
            logsOfBeforeMerge.Add(new LogOfMerge(UnityEditor.MessageType.Info, "The merge has been successfully completed! See merge statistics in the box below.\n\nProcessed in " + processingTime.Minutes + " minutes and " + processingTime.Seconds + " seconds."));
        ...
        private Texture2D GetSimpleCopyOfTexture(Material materialToFindTexture, string propertyToFindTexture, int widthOfCorrespondentMainTexture, int heightOfCorrespondentMainTexture, TextureType textureType, float progress, int texturesProcessedAtHere, int totalTexturesForProcess)
        {
            ...
            if (materialToFindTexture.HasProperty(propertyToFindTexture) == false || materialToFindTexture.GetTexture(propertyToFindTexture) == null)
            {
                ...
                //Launch log
                logsOfBeforeMerge.Add(new LogOfMerge(UnityEditor.MessageType.Warning, "It was not possible to find the texture stored in property \"" + propertyToFindTexture + "\" of material \"" + materialToFindTexture.name + "\", so this Texture/Map was replaced by a " + defaultColor.colorName + " texture. This can affect how the texture or effect maps (such as Normal Maps, etc.) are displayed in the combined model. This can result in some small differences in the combined mesh when compared to the separate original meshes."));
            ...
            catch (UnityException e)
            {
                if (e.Message.StartsWith("Texture '" + targetTexture.name + "' is not readable"))
                {
                    ...
                    //Launch logs
                    logsOfBeforeMerge.Add(new LogOfMerge(UnityEditor.MessageType.Warning, "It was not possible to combine texture \"" + targetTexture.name + "\" within an atlas, as it is not marked as \"Readable\" in the import settings (\"Read/Write Enabled\"). The texture has been replaced with a " + defaultColor.colorName + " one."));
                }
            }
        ...
        private void ApplyAtlasInPropertyOfMaterial(Material targetMaterial, string propertyToInsertTexture, Texture2D atlasTexture)
        {
            ...
            if (targetMaterial.HasProperty(propertyToInsertTexture) == false)
                logsOfBeforeMerge.Add(new LogOfMerge(UnityEditor.MessageType.Warning, "It was not possible to find and apply the atlas on property \"" + propertyToInsertTexture + "\" of the material to use (\"" + targetMaterial.name + "\"). Therefore, no atlas was applied to this property."));
        }
        ...
        private Texture2D GetTextureFilledWithColorOfMaterial(Material targetMaterial, string colorPropertyToFind, int width, int height)
        {
            ...
            {
                //Launch log
                logsOfBeforeMerge.Add(new LogOfMerge(UnityEditor.MessageType.Warning, "It was not possible to find the color stored in property \"" + colorPropertyToFind + "\" of material \"" + targetMaterial.name + "\", so this Color was replaced by a GRAY texture."));

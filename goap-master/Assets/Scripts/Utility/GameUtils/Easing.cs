...
namespace Dalechn
{
    ...
    public static class Easing
    {
        ...
        public static float CustomEase(EaseType type, AnimationCurve curve, float from, float to, float t)
        {
            if (type != EaseType.CustomCurve)
            {
                return Ease(type, from, to, t);
            }
            else
            {
                return Lerp(from, to, curve.Evaluate(Lerp(0, 1, t)));
            }
        }
